...
# create folder deployable from scratch and move all deployable items to it
mkdir deployable
cp app.yaml deployable/
# copy package.json for your node application, not the one for your react app
cp deploy/package.json deployable/
cp -r build deployable/
cp index.js deployable/

# rsync static objects hosted on cloud storage
gsutil -m rsync -r ./deployable/build/static/css gs://my-staging-bucket/build/static/css
gsutil -m rsync -r ./deployable/build/static/images gs://my-staging-bucket/build/static/images
gsutil -m rsync -r ./deployable/build/static/js gs://my-staging-bucket/build/static/js
...
gcloud app deploy deployable/app.yaml -q --version="fe-staging" --no-promote
